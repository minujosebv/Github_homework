...
plot(faithful$eruptions,faithful$waiting,pch=16,col="blue",
...
The above plot states that there is some linear relationship between the Eruption Time and Waiting Time. It can be seen that as the Duration of the eruption increases the Waiting Time for the next eruption also increases. So we can conclude that there is a positive correlation between the Duration of eruption and the waiting time for the next eruption. The above plot is a scatter plot with blue filled circle markers.

```{r}
#| echo: false
# plot
boxplot(faithful$waiting,col='cyan',
        main="Boxplot for Waiting Time",xlab="Waiting time")
```

The above box plot shows the distribution of waiting time. There is a minimum of 40-45 minutes to a maximum of 95-100 minutes with an average of 75-80 minutes of waiting time.
